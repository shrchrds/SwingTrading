...
def calculate_indicators(data, window_rsi=14, window_dma=20):
    """
    Calculate RSI and 20 DMA for a given DataFrame.
    Args:
    ...
    data['% Change from 20 DMA'] = ((data['Close'] - data['20 DMA']) / data['20 DMA'] * 100).round(2)
    return data
...
def calculate_daily_indicators(data, window_rsi=14, window_dma=200):
    ...
    data.bfill(inplace=True)  # Backfill to handle initial NA values
    data.ffill(inplace=True)  # Forward fill to handle any remaining NA values
